...
std::vector< Move > Plansza::pobierzMozliweRuchy(int x, int y)
{
	...
	//Jeœli pole na planszy puste zwracam pusty wektor
	...
	//Jeœli dla ruchu wybranym pionem nie ma bicia sprawdzCzyBicie(x, y) -> false
	//natomiast dla jakiegoœ innego piona jest bicie sprawdzCzyBicie(kolor) -> true
	//to zwracam pusty wektor poniewa¿ bicie jest obowi¹zkowe
	...
	else
	{
		...
		czyPionMaBicie = sprawdzCzyBicie(x, y);
		//sprawdzam dla ka¿dego kierunku
		for (int a : {-1, 1})
		{
			for (int b : {-1, 1})
			{
				int i = x + a;
				int j = y + b;
				//poruszaj siê a¿ pojawi siê coœ po drodze
				while (i >= 0 && j >= 0 && i <= 7 && j <= 7)
				{
					...
					{
						//jeœli na drodze pion tego samego koloru
						if (plansza[i][j] == BLACK_KING || plansza[i][j] == BLACK_PION)
						{
							break;
						}
						//na drodze pojawi³ siê pion przeciwnego koloru	
...
//zwraca 0 jeœli b³êdny ruch, 1 jeœli koniec ruchów, 2 jeœli jest jeszcze ruch
...
int Plansza::ruch(int x1, int y1, int x2, int y2, Color kolor)
{
	...
	bool czyBicie = false;
	//jeœli dany ruch dla tego pionka jest wykonalny
	...
	for (auto& move : pobierzMozliweRuchy(x1, y1))
	{
		if (move == Move(x1, y1, x2, y2))
		{
			...
			plansza[x1][y1] = EMPTY;
			// Jeœli nadal jest bicie dla tego piona
...
// zwraca 0 jeœli gra kontynuowana, 1 jeœli czerwone wygra³y, 2 jeœli czarne wygra³y, -1 jeœli remis 
...
//Zlicza iloœæ pionków i królowych pion 2 punkty, królowa 5 punktów
...
int Plansza::funkcjaOceniajacaPoziomy(int x, int y)
{
	...
	{
		if (x >= 2 && x <= 3)        //II poziom (3,4 rz¹d dla czerwonych)
		{
			return 1;
		}
		if (x >= 4 && x <= 5)        //III poziom (5,6 rz¹d)
		{
			return 3;
		}
		if (x >= 6 && x <= 7)        //IV poziom (7,8 rz¹d)
		{
			return 10;
		}
	}
	...
	{
		if (x >= 4 && x <= 5)        //II poziom (3,4 rz¹d dla czarnych)
		{
			return 1;
		}
		if (x >= 2 && x <= 3)        //III poziom (5,6 rz¹d)
		{
			return 3;
		}
		if (x >= 0 && x <= 1)        //IV poziom (7,8 rz¹d)
		{
			return 10;
		}
	}
...
//zwraca wartoœæ w zale¿noœci od stanu gry i sumy wartoœci pól
